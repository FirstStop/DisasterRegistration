...
Then(/^I should be asked for my basic information$/) do
  ...
  page.find('div[wizard-node="wizard.contact_details"] button[wizard-next]').click
  page.find('div[wizard-node="wizard.fun_fact"] button[wizard-next]').click
end

When(/^I decide to submit my application$/) do
  page.find('div[wizard-node="wizard.avatar"] button[wizard-submit]').click
end
